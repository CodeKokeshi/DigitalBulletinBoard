...
COMMENTS_FILE = os.path.join("static", "data", "comments", "comments.json")
...
async def display_announcement(announcement_id: int, request: Request, user: str = Depends(get_current_user)):
        ...
        for announcement in section:
            if announcement["announcement_id"] == announcement_id:
                announcement_comments = [
                    c for c in comments_data["comments"] if c["announcement_id"] == announcement_id
                ]
                likes = next(
                    (announcement["likes"]["amount"] for section in announcement_data.values() for announcement in
                     section if announcement["announcement_id"] == announcement_id),
                    0
                )
                return templates.TemplateResponse(
                    "announcement.html",
                    {
                        ...
                        # Pass the image_attachment to the template
                    }
                )
